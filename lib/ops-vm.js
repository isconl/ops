...
function createOpsVm({
  ...
  async function containerState(name) {
    ...
    // {{json .}} rather than {{json .State}} -- BI26091904 needs
    // RestartCount, which docker inspect puts at the container's top
    // level, not under .State.
    const r = await run('docker', ['inspect', id, '--format', '{{json .}}']);
    if (!r.ok) return { exists: false, running: false };
    try {
      const c = JSON.parse(r.stdout);
      const state = c.State || {};
      return {
        exists: true, running: !!state.Running, status: state.Status, startedAt: state.StartedAt,
